...
2. **Build a shell-safe invocation.** Single-quote the raw goal; double quotes
   ...
   ```bash
   node '<plugin root>/scripts/circuit.ts' run explore --goal 'can'\''t go' --progress jsonl
   ```
   ...
   ```bash
   node '<plugin root>/scripts/circuit.ts' run explore --goal 'find deprecated APIs' --progress jsonl
   ```
