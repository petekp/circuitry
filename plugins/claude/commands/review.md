...
2. **Build a shell-safe invocation.** Single-quote the raw scope text. Double quotes expand `$VAR`,
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/circuit.ts" present run review --goal 'review the latest change'
   ```

   Example with an apostrophe:

   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/circuit.ts" present run review --goal 'can'\''t regress runtime safety'
   ```
