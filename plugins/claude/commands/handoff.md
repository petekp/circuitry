...
3. **Save mode.** Infer a concise goal, next action, state, and debt from the
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/circuit.ts" present handoff save --goal '<goal>' --next '<next action>' --state-markdown '<state bullets>' --debt-markdown '<debt bullets>'
   ```
...
6. **Brief mode.** Run:
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/scripts/circuit.ts" handoff brief --json
   ```
