...
async function loadSchematicsFromCatalog(): Promise<SchematicEntry[]> {
  ...
  try {
    ...
    return mod.flowPackages.map((pkg) => ({
      ...
      definitionSourcePath: `src/flows/${pkg.id}/facts.ts`,
...
function renderSurfaceInventory(): string {
  const rows = [
    ...
    [
      ...
      '`src/flows/<id>/facts.ts` + `src/flows/<id>/flow.ts`',
      '`npm run build && node scripts/emit-flows.ts`',
      'no',
      '`src/flows/<id>/schematic.json`',
      '`node scripts/emit-flows.ts --check`',
      'JSON schematics are retained for compatibility and generated from typed flow facts plus the flow adapter.',
    ],
    [
...
async function renderGeneratedSurfaceMap(): Promise<string> {
  ...
  return `${[
    ...
    '## Edit Rules',
    '',
    '- Flow facts are authored in `src/flows/<id>/facts.ts`; `src/flows/<id>/flow.ts` binds those facts to routing, schemas, writers, and hints.',
