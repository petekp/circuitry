...
snapshot. This is the check every entry point runs before treating a
folder as resumable. Status projection, checkpoint resume (§17), and
handoff continuity all read this same contract.

This is more than tidiness. It means every report a run can produce —
...
A `result.json` is the machine-facing report of a run. An operator
