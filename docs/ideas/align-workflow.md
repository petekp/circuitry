# Align workflow — operator/model intent alignment

Idea for a Circuit workflow dedicated to establishing and maintaining
...
The align workflow exists to make that foundation explicit, durable,
and actively consulted.

## Grounded in validated practice

This workflow is not hypothesis. It formalizes a habit the operator
...
The align workflow exists to (a) make this practice repeatable and
structured, (b) preserve its outputs as durable artifacts the codebase
can carry forward, and (c) wire those artifacts into future runs so
...
the result, then explicit wiring so the artifacts get consulted in
future runs.
...
- **Interrogate** — the heart. Agent grills operator on: success,
  ...
  minimize.
- **Synthesize** — draft artifacts AND preserve the raw interrogation
  transcript as a first-class artifact alongside them. The distilled
  ...
  the distilled version is ambiguous. Synthesis is real thinking
  about implications and contradictions, not transcription.
- **Stress-test** — run artifacts against hypothetical decisions.
...
- **Wire** — decide how the artifacts get consulted in future runs.
  The often-skipped step. Without this, the artifacts become
  decoration.
- **Close** — artifacts land, pointers added, optional recurring
  audit-self cadence scheduled.

## The artifacts — fewer is better
...
The operator picks which artifacts matter for the project at hand.
Empty placeholders are forbidden — if a doc isn't worth filling in,
don't produce it.

## Interrogation UX — improve on grill-me

`grill-me` proved the value of deep interrogation. The align workflow
should learn from it and improve on the UX, hooking into adapter-
specific UI primitives where available. In Claude Code that means
...
caught before artifacts are drafted. Close predictions = alignment
already high. Either way, more signal than free-text alone.

**Specific UX upgrades to design for:**

- **Predicted-answer Q/A as the default primitive.** For each
...
- **Preview affordance on load-bearing options.** Render a snippet of
  what the *artifact* would say under each option ("if you pick A,
  the principles doc would commit to X"). Operator picks between
  concrete consequences, not abstractions. Catches drift before
  synthesis, not after.
- **Themed batches with incremental synthesis.** Chunk into clusters
  (audience, success/failure, tradeoffs, refusals, anti-patterns),
  4–8 questions per batch. Synthesize between batches so later
...
- **Live artifact preview between batches.** Operator sees the
  artifacts taking shape and can catch "wait, that's not what I
  meant" before final synthesis.
- **Right primitive per question.** Not every question wants to be
  multiple-choice. Some questions genuinely need free-text depth
  (e.g., "describe the user you're building for") and forcing
  multiple-choice would flatten the value. The workflow picks the
  right primitive per question: predicted-answer Q/A when the agent
...
primitive. Other adapters (Codex, future host integrations) will need
graceful degradation to terminal prompting or other mechanisms.
Design the workflow with the rich UI as the preferred path, not the
only path.

## The drift-prevention mechanism — the part that matters most

Most vision-doc efforts fail because the docs exist and nothing
consumes them. The artifacts only prevent drift if a mechanism uses
them. Options:

- **Auto-pulled at Frame** — every workflow's Frame phase reads them
  in. Simple but noisy on small tasks.
- **Decision-checkpoint** — when a workflow hits a load-bearing
  decision, it explicitly checks against the align artifacts. Composes
...
- **Embedded in rigor** — Deep rigor pulls align artifacts; Lite
  skips. Avoids noise on small tasks.

The drift detector is the mechanism that would justify the upfront
investment. Without it, the workflow is theater.
...
trims interrogation depth produces lighter artifacts that don't carry
the alignment weight the project needs. The workflow's failure mode
isn't "too long" — it's a short-circuited operator who answers
questions superficially. The workflow should be willing to refuse to
produce artifacts if engagement is shallow.
...
believe. Artifacts should be living, with explicit "revisit on X
trigger" markers — not frozen.

**Pedagogical bonus, not just artifact production.** Structured
...
the workflow, not just a side effect.

## Naming note

This workflow has the strongest claim on the name `align` (aligning
operator and model intent — close to the dictionary meaning). The
earlier idea of an `align` workflow for converging divergent code
...
- **Relates to `learn` step** (see `self-improving-circuit.md`) —
  `learn` could be the mechanism that keeps align artifacts current
  over time. End-of-run evaluation: "did anything we learned today
  invalidate or strengthen something in the align artifacts?"
- **Relates to `recall` step** — `recall` at Frame would naturally
  pull align artifacts into context. The wiring question above is
  partly answered by an existing `recall` design.
- **Relates to `audit-self`** — periodic check that the align
  artifacts haven't drifted from current practice.

## What to prototype

Before building the workflow, prove the concept by running it manually
...
align artifacts would test whether the synthesis adds real value or
just rearranges what's there.

Steps for the manual prototype:

1. Operator and agent run the Surface phase by hand — list every
...
2. Run a compressed Interrogate session against gaps and contradictions,
   ...
   primitive.
3. Draft the five artifact candidates (or fewer, if some don't earn
   their place).
4. Stress-test against three real recent decisions from circuit-next
   ...
   made?
5. If yes, the workflow has signal. Build it.
6. If no, the bar for what counts as a useful artifact needs rework
   before tooling investment.

## Open questions

- How does the workflow handle disagreement between operator answers
  given on different days? (Versioning? Re-interrogation? Last-answer-
  wins?)
- Should align artifacts be checked into the repo, or kept in a
  separate operator-private location? Some artifacts feel
  team-shareable, others feel like private operator notes.
- What's the right cadence for `audit-self` against align artifacts?
  Triggered by N runs? Calendar interval? Detected drift?
- For multi-operator projects (future), how does this workflow handle
...
- For non-Claude-Code adapters, what's the degraded interrogation UX?
  ...
  designing the workflow's question-spec format to be primitive-
