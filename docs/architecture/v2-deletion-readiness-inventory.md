...
| `src/runtime/catalog-derivations.ts` | compatibility wrapper | Neutral implementation moved to `src/flows/catalog-derivations.ts` in Phase 5.13. Keep old path for retained imports, tests, and external compatibility. |
...
execution, retained/v1 checkpoint, connector/materializer, router/compiler, or
old oracle-test deletion safe.
