...
| Runtime support and progress metadata are package-owned. | Confirmed | `src/flows/catalog.ts` derives a runtime-surface map from `flowPackages`; `src/cli/circuit.ts` reads that metadata for depth support and progress. |
...
| Connector sharing is intentionally narrow. | Confirmed | `src/shared/connector-relay.ts` owns the neutral relay/hash contract; connector modules import it directly. `src/connectors/subprocess.ts` owns subprocess lifecycle. `tests/contracts/architecture-boundaries.test.ts` ratchets this boundary. |
