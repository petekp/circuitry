...
2. Validate Repair scaffolding exists: `circuit.manifest.yaml`, `events.ndjson`, the derived `state.json` snapshot, and `artifacts/active-run.md`.
3. Report the validated run root and scaffold state briefly.
4. Stop here. Do not continue into Frame/Analyze/Fix/Verify/Review/Close or do unrelated repo exploration.
