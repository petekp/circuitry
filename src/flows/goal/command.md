...
1. **Build a shell-safe invocation.** Single-quote the raw goal text instead of
   ...
   ```bash
   ./bin/circuit run goal --goal 'ship the scoped objective' --progress jsonl
   ```

   Lite Goal:

   ```bash
   ./bin/circuit run goal --goal 'ship the scoped objective' --rigor lite --progress jsonl
   ```

   Deep Goal:

   ```bash
   ./bin/circuit run goal --goal 'ship the scoped objective' --rigor deep --progress jsonl
   ```

   Autonomous Goal:

   ```bash
   ./bin/circuit run goal --goal 'ship the scoped objective' --autonomous --progress jsonl
   ```
