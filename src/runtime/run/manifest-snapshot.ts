// Runtime manifest snapshot persistence. A run stores the exact compiled-flow
// bytes it started with so resume and status projection use the same flow even
// if generated files change later. Graph execution only passes the bytes
// through.
