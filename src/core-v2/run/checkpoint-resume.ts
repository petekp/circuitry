...
import type { CheckpointStepV2, ExecutableFlowV2 } from '../manifest/executable-flow.js';
...
import { runCompiledFlowChildV2 } from './compiled-flow-runner.js';
import {
  type GraphRunResultV2,
  executeExecutableFlowV2,
  isGraphCheckpointWaitingResultV2,
} from './graph-runner.js';
import { readCompiledFlowManifestSnapshotV2 } from './manifest-snapshot.js';

export interface ResumeCompiledFlowOptionsV2 {
...
interface V2CheckpointRequestContext {
...
function traceString(entry: TraceEntryV2 | undefined, key: keyof TraceEntryV2): string | undefined {
  const value = entry?.[key];
  return typeof value === 'string' && value.length > 0 ? value : undefined;
}

function traceDataString(entry: TraceEntryV2 | undefined, key: string): string | undefined {
...
function v2TraceString(
  entry: TraceEntryV2 | undefined,
  key: keyof TraceEntryV2,
): string | undefined {
...
function isCoreV2Bootstrap(entry: TraceEntryV2 | undefined): entry is TraceEntryV2 {
  return (
    entry?.kind === 'run.bootstrapped' &&
    v2TraceString(entry, 'engine') === 'core-v2' &&
    v2TraceString(entry, 'manifest_hash') !== undefined
  );
}

export async function isCoreV2RunFolder(runDir: string): Promise<boolean> {
  try {
    const trace = new TraceStore(runDir);
    const entries = await trace.load();
    return isCoreV2Bootstrap(entries[0]);
  } catch {
    return false;
  }
}

function latestUnresolvedCheckpoint(entries: readonly TraceEntryV2[]): TraceEntryV2 {
  ...
  throw new Error('core-v2 checkpoint resume rejected: run has no unresolved checkpoint request');
}
...
function checkpointStep(input: {
  ...
  if (step === undefined || step.kind !== 'checkpoint') {
    throw new Error(
      `core-v2 checkpoint resume rejected: current step '${input.stepId}' is not a checkpoint`,
    );
  }
  return step;
}

function declaredCheckpointRequestPath(step: CheckpointStepV2): string {
  const requestPath = step.writes?.request?.path;
  if (requestPath === undefined) {
    throw new Error(
      `core-v2 checkpoint resume rejected: checkpoint step '${step.id}' has no declared request path`,
    );
  }
  return requestPath;
}
...
function readCheckpointRequestContext(input: {
  ...
  if (sha256Hex(requestText) !== input.expectedRequestHash) {
    throw new Error(
      'core-v2 checkpoint resume rejected: checkpoint request hash differs from trace',
    );
  }
  const raw = JSON.parse(requestText) as unknown;
  if (!isRecord(raw)) {
    throw new Error(
      `core-v2 checkpoint resume rejected: request for '${input.step.id}' is invalid`,
    );
  }
  if (raw.schema_version !== 1 || raw.step_id !== input.step.id) {
    throw new Error(`core-v2 checkpoint resume rejected: request for '${input.step.id}' is stale`);
  }
  ...
  if (
    ...
    throw new Error(
      `core-v2 checkpoint resume rejected: request choices for '${input.step.id}' are stale`,
    );
  }
  const context = raw.execution_context;
  if (!isRecord(context)) {
    throw new Error(
      `core-v2 checkpoint resume rejected: request for '${input.step.id}' has no execution context`,
    );
  }
  const projectRoot = context.project_root;
  if (projectRoot !== undefined && typeof projectRoot !== 'string') {
    throw new Error('core-v2 checkpoint resume rejected: project_root is invalid');
  }
  ...
  if (checkpointReportSha256 !== undefined && typeof checkpointReportSha256 !== 'string') {
    throw new Error('core-v2 checkpoint resume rejected: checkpoint_report_sha256 is invalid');
  }
  return {
...
function validateCheckpointReport(input: {
  ...
  readonly requestContext: V2CheckpointRequestContext;
}): void {
  const report = input.compiledStep.writes.report;
  if (report === undefined) {
    if (input.requestContext.checkpointReportSha256 !== undefined) {
      throw new Error(
        `core-v2 checkpoint resume rejected: checkpoint '${input.compiledStep.id}' request carries a report hash but the step writes no report`,
      );
    }
    return;
  }
  if (typeof report === 'string') {
    if (input.requestContext.checkpointReportSha256 !== undefined) {
      throw new Error(
        `core-v2 checkpoint resume rejected: checkpoint '${input.compiledStep.id}' request carries a report hash but the report has no schema validator`,
      );
    }
    return;
  }
  ...
  if (builder?.validateResumeContext === undefined) {
    if (input.requestContext.checkpointReportSha256 !== undefined) {
      throw new Error(
        `core-v2 checkpoint resume rejected: builder for schema '${report.schema}' is missing validateResumeContext but the checkpoint request carries a report hash`,
      );
    }
    return;
  }
...
function executableFlowForResume(input: {
  ...
  readonly bootstrap: TraceEntryV2;
}): ExecutableFlowV2 {
  const executable = fromCompiledFlowV1(input.flow);
  ...
  return {
    ...
    metadata: {
      ...
      ...(v2TraceString(input.bootstrap, 'depth') === undefined
        ? {}
        : { selected_depth: v2TraceString(input.bootstrap, 'depth') }),
    },
  };
}

export async function resumeCompiledFlowV2(
  options: ResumeCompiledFlowOptionsV2,
  ...
  if (!isCoreV2Bootstrap(bootstrap)) {
    throw new Error('core-v2 checkpoint resume rejected: run folder is not marked core-v2');
  }
  if (entries.some((entry) => entry.kind === 'run.closed')) {
    throw new Error('core-v2 checkpoint resume rejected: run is already closed');
  }
  ...
  if (
    ...
    throw new Error('core-v2 checkpoint resume rejected: bootstrap identity is incomplete');
  }

  const { flow, flowBytes, snapshot } = await readCompiledFlowManifestSnapshotV2({
  ...
  if (
    ...
    throw new Error('core-v2 checkpoint resume rejected: checkpoint request trace is incomplete');
  }
  ...
  if (!sameStringArray(allowedChoices, savedChoices)) {
    throw new Error(
      `core-v2 checkpoint resume rejected: checkpoint trace choices for '${stepId}' are stale`,
    );
  }
  if (!savedChoices.includes(options.selection)) {
    throw new Error(
      `core-v2 checkpoint resume rejected: selection '${options.selection}' is not allowed for checkpoint '${stepId}'`,
    );
  }
  ...
  if (compiledStep === undefined || compiledStep.kind !== 'checkpoint') {
    throw new Error(`core-v2 checkpoint resume rejected: saved flow step '${stepId}' is invalid`);
  }
  const allowed = (step.check as { readonly allow?: unknown }).allow;
  if (Array.isArray(allowed) && !allowed.includes(options.selection)) {
    throw new Error(
      `core-v2 checkpoint resume rejected: selection '${options.selection}' is outside check.allow for checkpoint '${stepId}'`,
    );
  }
  const declaredRequestPath = declaredCheckpointRequestPath(step);
  if (requestPath !== declaredRequestPath) {
    throw new Error(
      `core-v2 checkpoint resume rejected: checkpoint request path '${requestPath}' does not match saved flow path '${declaredRequestPath}'`,
    );
  }
  ...
  const depth = v2TraceString(bootstrap, 'depth');

  const result = await executeExecutableFlowV2(executable, {
    runDir: options.runDir,
  ...
  if (isGraphCheckpointWaitingResultV2(result)) {
    throw new Error('core-v2 checkpoint resume rejected: resume did not resolve checkpoint');
  }
  return result;
}
